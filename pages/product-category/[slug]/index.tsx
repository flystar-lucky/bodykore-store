...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  return {
    props: { shopifyData, header, CMSData },
    revalidate: 30 * 60,
  };
};
...
const Category = ({ shopifyData, header, CMSData }: CategoryParams) => {
  console.log('hellpo');
  const mapSubcategories = () => {
    return CMSData.category!.subcategories.map((item) => ({
      ...
      topTitle: item.name,
      title: item.name,
      link: `${routes.products.path}?category=${shopifyData.title}&subcategory=${item.name}`,
    }));
  };
  ...
  return (
    <>
      ...
      <section className="w-full">
        ...
        <div className="pt-44 px-8" id="belowBanner">
          <TextRImg
            title="DISCOVERING"
